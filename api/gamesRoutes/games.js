...
async function getAllGames(summonerid, beginningIndex, res) {
  try {
    let response = await axios.get(
      `https://na1.api.riotgames.com/lol/match/v4/matchlists/by-account/${summonerid}?queue=420&queue=%20440&beginIndex=${beginningIndex}&api_key=${api_key}`
    );
...
async function getSingleGames(gamesData, summonerid, res) {
  ...
  try {
    while (startIndex < endIndex) {
      console.log(counter);
      let response = await axios.get(
        `https://na1.api.riotgames.com/lol/match/v4/matches/${matches[counter].gameId}?api_key=${api_key}`
      );
      ...
      for (let i = 0; i < response.data.participantIdentities.length; i++) {
        if (
          response.data.participantIdentities[i].player.accountId == summonerid
        ) {
          if (i < 5) {
            redBlue = true;
          } else {
      ...
      db("games")
        .where({ gameid: response.data.gameId })
        .first()
        .then(id => {
          if (id) {
